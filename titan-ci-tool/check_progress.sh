...
EOF
  ...
  # Try to convert markdown to PDF using pandoc
  if command -v pandoc >/dev/null 2>&1; then
    echo "Converting report to PDF using pandoc..."
    pandoc security_report.md \
      ...
      --toc \
      --toc-depth=2 \
      2>/dev/null && echo "PDF report saved as security_report.pdf" || {
        echo "Pandoc PDF conversion failed, trying alternative method..."
        # Fallback: try with xelatex if available
        if command -v xelatex >/dev/null 2>&1; then
          pandoc security_report.md \
            ...
            --variable colorlinks=true \
            2>/dev/null && echo "PDF report saved as security_report.pdf" || {
              echo "PDF conversion failed, saving as markdown only"
              echo "Markdown report saved as security_report.md"
            }
        else
          echo "PDF conversion tools not available, saving as markdown only"
          echo "Markdown report saved as security_report.md"
        fi
      }
  else
    echo "Pandoc not available for PDF conversion, saving as markdown only"
    echo "Markdown report saved as security_report.md"
  fi
